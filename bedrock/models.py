...
class ChatModel:
    def __init__(self, model_name: str, model_kwargs: Dict):
        ...
        if "mistral" in self.model_id:
            ...
            self.llm = ChatBedrockConverse(model=self.model_id, max_tokens=self.model_kwargs["max_tokens"], temperature=self.model_kwargs["temperature"], top_p=self.model_kwargs["top_p"], additional_model_request_fields={"inferenceConfig": {"topK": self.model_kwargs["top_k"]}})
        else:
